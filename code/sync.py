...
def main(_):
    ...
    if job_name == 'ps':
        ...
        FLAGS.num_workers = len(FLAGS.worker_hosts.split(","))
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/replica:0/task:{}".format(task_index), cluster=cluster)):
            ...
            # optimizer = tf.train.AdamOptimizer(FLAGS.lr).minimize(cost, global_step=global_step)
            # optimizer = tf.train.GradientDescentOptimizer(FLAGS.lr)
            optimizer = tf.train.AdamOptimizer(FLAGS.lr)
            sync_optimizer = tf.train.SyncReplicasOptimizer(optimizer, replicas_to_aggregate = replicas_to_aggregate, total_num_replicas = FLAGS.num_workers, name="CFMF")
            sync_replicas_hook = sync_optimizer.make_session_run_hook(is_chief)
            hooks = [sync_replicas_hook]
            train_step = sync_optimizer.minimize(cost, global_step=global_step)
