...
def main(_):
    ...
    if job_name == 'ps':
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/replica:0/task:{}".format(task_index), cluster=cluster)):
            ...
            # grad_op = tf.train.GradientDescentOptimizer(FLAGS.lr)
            grad_op = tf.train.AdamOptimizer(FLAGS.lr)
            rep_op = tf.contrib.opt.DropStaleGradientOptimizer(grad_op, staleness=10, use_locking=True) 
            train_step = rep_op.minimize(cost,global_step=global_step)
